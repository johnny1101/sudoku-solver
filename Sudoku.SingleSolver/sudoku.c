...
int IsValidGrid(grid_t grid) {
   ...
   for (j = 0; j < SIZE; j++) {
      ...
      oy = j % N;
      // check row
      if (y != j && v == grid->matrix[x * SIZE + j]) return 0;

      // check column
      if (x != j && v == grid->matrix[j * SIZE + y]) return 0;
...
int Prepare_allotment_stack(grid_t grid) {
    ...
    stack_t stack1 = Alloc_stack();
    stack_t stack2 = Alloc_stack();
    stack_t allot_stack1;
    stack_t allot_stack2;
    ...
    while(!IsEmptyStack(allot_stack1)) {
...
int **solveSudoku(int ** original_matrix) {
    ...
    if(r >= 0) {
        ...
        if(r == 0) {
            ...
            for(i = 0; i < allotment_stack->list_sz && !soln_found; i++) {
